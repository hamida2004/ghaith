...
exports.getUnassignedDonations = async (req, res) => {
  try {
    ...
    console.error(
      "GET UNASSIGNED DONATIONS ERROR:",
      err
    );
...
exports.createDonation = async (req, res) => {
  try {
    ...
    if (
      ...
      return res.status(400).json({
        msg:
          "Cannot donate to non-approved request"
      });
    }
...
exports.dispatchDonation = async (req, res) => {
  ...
  try {
    ...
    if (
      ...
      return res.status(400).json({
        msg:
          "request_id and allocated_amount are required"
      });
    }
    ...
    if (
      ...
      return res.status(400).json({
        msg:
          "Only free donations can be dispatched"
      });
    }
    ...
    if (
      ...
      return res.status(400).json({
        msg:
          "Donation is not in pending_assignment status"
      });
    }

    const usedResult =
      await db.Donation.sum(
        "allocated_amount",
        {
          where: {
            parent_id:
    ...
    if (
      ...
      return res.status(400).json({
        msg:
          `Cannot allocate ${parsedAllocated}. Only ${remaining.toFixed(2)} remaining.`
      });
    }
    ...
    res.status(201).json({
      msg:
        "Dispatch created — seeker will now validate",
...
exports.updateDonationStatus = async (req, res) => {
  ...
  try {
    ...
    if (
      ...
      return res.status(400).json({
        msg:
          "Donation not ready for seeker validation"
      });
    }
    ...
    if (status === "confirmed") {
      ...
      const effectiveAmount =
        parseFloat(
          donation.allocated_amount ??
          donation.amount
        );

      const collected =
        parseFloat(
          lockedRequest.collected_amount || 0
        );
