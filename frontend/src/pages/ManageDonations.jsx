...
export const ManageDonations = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const eligibleRequests = requestRes.data.filter(
        (r) =>
          r.status === "accepted" &&
  ...
  const handleDispatch = async (donationId) => {
    try {
      ...
      await axios.post(
        ...
        {
          request_id: dispatchRequest[donationId],
          allocated_amount: dispatchAmount[donationId]
        }
      );
      ...
      alert(
        err.response?.data?.msg || "Dispatch failed"
      );
    }
  };
  ...
  // REMAINING
  // =====================
  const computeRemaining = (donation) => {

    const dispatches = donation.Dispatches || [];

    const used = dispatches.reduce(
      (sum, d) =>
        sum +
        parseFloat(
          d.allocated_amount || d.amount || 0
        ),
      0
    );

    return (
      parseFloat(donation.amount || 0) - used
    ).toFixed(2);
  };
